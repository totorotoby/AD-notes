...
\node[squarednode] (y1) at (3.75,-5) {$y_1 = v_4 - v_1$};
...
\node[squarednode] (y) at (3.75,-5) {$\pd{y_1}{x} = \pd{v_4}{x} - \pd{v_1}{x}$};
...
Reverse mode AD is more confusing, but solves the problem about calculating the jacobian quickly if $m \ll n$. The intuation behind reverse mode is to ignore the fact that $x$ are inputs and that $y$ are outputs in the forward model. Now, instead of seeding the inputs, we instead seed the outputs and use the chain rule in the reverse direction up the graph we get something like this:
...
\node[roundnode] (y2in) at (6.75,-7) {$\pd{y}{y_2}$};
\node[roundnode] (y1in) at (3.75,-7) {$\pd{y}{y_1}$};

\node[squarednode] (y2) at (6.75,-5) {$\pd{v_6}{x} = \pd{v_3}{x}$};
\node[squarednode] (y1) at (3.75,-5) {$\pd{v_5}{x} = \pd{v_4}{x} - \pd{v_1}{x}$};
